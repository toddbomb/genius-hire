      ...
      {props.files.length > 0 && (
        <div className="flex flex-col items-left justify-center w-full mt-4">
          <div className="flex flex-row">
            <div
              className="rounded-md flex shadow p-2 mb-2 w-full bg-[#6e7173] items-center cursor-pointer"
              onClick={handleListExpand}
            >
              {props.title}
              <div className="bg-[#93969a] ml-2 px-2 rounded-full w-max text-center text-sm ">
                {props.files.length}
              </div>
            </div>
